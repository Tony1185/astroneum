/**
 * TypedArrayIndicators — SIMD-friendly column-store indicator calculations
 * implemented in pure TypeScript.
 *
 * History: this module was previously named `WasmIndicators`. The name was
 * inherited from a planned Rust/`wasm-pack` implementation that never
 * shipped. It was renamed in v0.3.0 to reflect what the code actually does.
 * A back-compat `WasmIndicators.ts` re-export remains until v1.0.
 *
 ...
 *   A real WASM+SIMD path is tracked under roadmap item #7.
