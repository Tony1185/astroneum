...
import ScriptEngine, { type CompiledIndicator } from '../scripting/ScriptEngine'

describe('ScriptEngine', () => {
  it('auto-registers compiled indicators', () => {
    const name = `UNIT_SCRIPT_PLUGIN_${Date.now()}`
    const engine = ScriptEngine.getInstance()
    const template: CompiledIndicator = engine.compile(`
      study('${name}', { overlay: true })
      plot(close, { title: 'Close' })
    `, name)

    assert.equal((template as unknown as { name: string }).name, name)
